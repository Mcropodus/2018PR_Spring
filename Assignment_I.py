...
x_train, x_test, y_train, y_test = ms.train_test_split(
    img_pixel, array, test_size=0.1
)
model_naive = naive_bayes.GaussianNB()  # Gaussian model
model_naive.fit(x_train, y_train)
model_naive.fit(n_img_pixel, n_array)
...
y_proba = model_naive.predict(test)
# print(y_proba)
result = y_proba.reshape(t_height, t_width)
